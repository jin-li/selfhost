...
1. In `.env`, set the Cloudflare API token and email for authentication. Set acme email for TLS.
2. Create `dynamic` folder based on `dynamic.example` for hosting the file-based dynamic configurations. Add your desired routes and services in the `dynamic` folder. You can refer to the example configuration files in `dynamic.example` for guidance.
3. Run `docker compose up -d` to deploy the container.
